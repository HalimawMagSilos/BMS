        ...
        if ($user && password_verify($current_password, $user['password'])) {
            if ($new_password === $confirm_password) {
                if (validatePassword($new_password)) {
                    ...
                    $action = "Barangay Official (Financial Assistance) Changed password for user ID: " . $_SESSION['user_id'];
                    $log_stmt = $pdo->prepare("INSERT INTO System_Logs (user_id, action) VALUES (?, ?)");
                    $log_stmt->execute([$_SESSION['user_id'], $action]);

                    // Redirect or show success message
                    header("Location: FASettings.php?success=1");
                    exit();
                } else {
...
<body class="bg-gray-100 flex">
    ...
    <main id="main-content" class="main-content flex-grow p-8 ml-0">
        ...
        <div class="bg-white p-8 rounded-lg shadow-md mb-8">
            ...
            <form action="FASettings.php" method="POST">
                <div class="mb-4">
